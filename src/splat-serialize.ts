...
const serializeViewer = async (splats: Splat[], options: ViewerExportOptions, write: WriteFunc) => {
    ...
    const viewerSettings: ViewerSettings = {
        ...
        background: {
            color: [ bgClr[0], bgClr[1], bgClr[2] ]
        },
    };

    const html = ViewerHtmlTemplate
    .replace('{{contentURL}}', options.type === 'html' ? `data:application/ply;base64,${encodeBase64(compressedData)}` : 'scene.compressed.ply')
    .replace('{{viewerSettingsURL}}', options.type === 'html' ? `data:application/json;base64,${encodeBase64(new TextEncoder().encode(JSON.stringify(viewerSettings)))}` : './viewer-settings.json');
    ...
    if (options.type === 'html') {
        ...
        zip.file('viewer-settings.json', JSON.stringify(viewerSettings, null, 4));
