...
class AboutPopup extends Overlay {
    constructor(args = {}) {
        ...
        // Header row (logo + title/version, centered)
        ...
        logoContainer.class.add('about-link');
        ...
        // Title container (name + version)
        const titleContainer = new Container({
            id: 'about-title-container'
        });
        titleContainer.class.add('about-link');
        titleContainer.dom.addEventListener('click', () => {
            window.open('https://github.com/playcanvas/supersplat', '_blank')?.focus();
        });
        ...
        titleContainer.append(appName);
        titleContainer.append(appVersionLabel);

        headerRow.append(logoContainer);
        headerRow.append(titleContainer);
        ...
        pcuiRow.class.add('about-dep-link');
        const pcuiNameLabel = new Label({
            class: 'about-dep-name',
            text: 'PCUI'
        });
        const pcuiVersionLabel = new Label({
            class: 'about-dep-version',
            text: `v${pcuiVersion}`
        });
        const pcuiRevisionLabel = new Label({
            class: 'about-dep-revision',
            text: `(${pcuiRevision.substring(0, 7)})`
        });
        pcuiRow.append(pcuiNameLabel);
        pcuiRow.append(pcuiVersionLabel);
        pcuiRow.append(pcuiRevisionLabel);
        ...
        engineRow.class.add('about-dep-link');
        const engineNameLabel = new Label({
            class: 'about-dep-name',
            text: 'PlayCanvas'
        });
        const engineVersionLabel = new Label({
            class: 'about-dep-version',
            text: `v${engineVersion}`
        });
        const engineRevisionLabel = new Label({
            class: 'about-dep-revision',
            text: `(${engineRevision.substring(0, 7)})`
        });
        engineRow.append(engineNameLabel);
        engineRow.append(engineVersionLabel);
        engineRow.append(engineRevisionLabel);
