...
const registerTrackManagerEvents = (events: Events) => {
    ...
    const trackEdit = (name: string, editFn: (track: AnimTrack) => void) => {
    ...
    events.on('track.copyKey', (fromFrame: number, toFrame: number) => {
        trackEdit('copyKey', (track) => {
            track.copyKey(fromFrame, toFrame);
        });
    });
};
